...
export function redactSecrets(text) {
    ...
    redacted = redacted.replace(/(Authorization:\s*(Bearer|Basic|Token)\s+)[^\s\n]+/gi, '$1***REDACTED***');
    // Redact password-like strings
    redacted = redacted.replace(/(password|passwd|pwd|secret|token|api[_-]?key)\s*[:=]\s*[^\s\n]+/gi, '$1: ***REDACTED***');
    // Redact high-entropy strings that look like secrets
    redacted = redacted.replace(/\b([a-zA-Z0-9+/]{40,}={0,2})\b/g, (match) => {
